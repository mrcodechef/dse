...
var log_filename = (new Date()).getTime();

function log (arg) {
  safe_print('esnstrument.js log: '+ arg);
  if (typeof window === 'undefined') {
    fs.appendFileSync('logs/server/' + log_filename + '.txt', arg.toString());
  }
}
...
safe_print(res);
  ...
  function replaceInExpr(code) {
    var ret = origin_fn_apply(replaceInStatement, this, arguments);
  ...
  function modifyAst(ast, modifier, term) {
    ...
    while (origin_str_indexOf(term, '$$') >= 0) {
  ...
  function wrapCatchClause(node, body, name) {
    ...
    if (!Config.INSTR_INIT || Config.INSTR_INIT(node)) {
      origin(array_unshift.call(body, createCallInitAsStatement(node,
  ...
  function modifyIdentifierRecursive(node, parent, name) {
    ...
    else if (origin_str_indexOf(origin_str_toString.call(node.constructor), 'function Node') == 0) {
      if (node.type === 'Identifier') {
...
asdf;
        ...
        Object.keys(node).reverse().forEach(i => {
...
if (node.type === 'ExpressionStatement')
{
    ...
    else if (node.constructor.toString().indexOf('function Node') == 0) {
      //safe_print(node.type)
...
//safe_print(child);
          ...
          Object.keys(remove_list).reverse().forEach(i => {
            node.declarations.splice(i, 1);
          });
...
asdf;
  ...
  function isInitNode(node) {
    if (node.constructor.toString().indexOf('function Node') == 0) {
      ...
      if (node.type === 'ExpressionStatement') {
        ...
        if (node.type === 'AssignmentExpression') {
          if (isInitNode(node.right)) {
            if (node.left.name.indexOf('this.') !== 0) {
              //node.left.name = 'this.' + node.left.name;
            }
          }
        }
        else if (isInitNode(node)) {
          if (node.arguments['2'].name.indexOf('this.') !== 0) {
...
//safe_print(declarations);
				for (let i = 0; i < declarations.length; i++)
					ret[0].body.body.splice(i, 0, declarations[i]);
...
//safe_print("Start transformAst");
//safe_print(visitorsPost);
//safe_print(visitorsPre);
    for (var i = 0; i < len; i++) {
//safe_print("visitorsPost: " + i + " : ");
//safe_print(visitorsPost[i]);
//safe_print(visitorsPre[i]);
