...
function log (arg) {
  ...
  if (typeof window === 'undefined') {
    fs.appendFileSync('logs/server/' + log_filename + '.txt', origin_obj_toString.call(arg));
  }
}
...
console.log("Check");
console.log(replaceInStatement);
...
//safe_print(child);
  ...
  function isInitNode(node) {
    if (origin_str_indexOf.call(origin_obj_toString.call(node.constructor), 'function Node') == 0) {
...
safe_print("Wrap Script Body");
      if (node.scope.parent == null) {
        ...
        let body_node = ret[0].body.body[0].body.body.body[1].block.body[0].expression.callee.object.callee.object.body.body;
        Object.keys n(body_node).reverse (). forEach (i => {
...
*/  
  ...
  /*
   ...
   //			// function
   //			r = function(){ return src.appl y(this, arguments); };
   }else{
   // generic objects
   try {
...
///safe_print("Function Decl Hoisted");
          ...
          if (newBody.length !== i + 1) {
            //hoisteredFunctions.push n(ast.body[i].id.name);
          }
        }
...
safe_print("Hoist Loop End " + ast.type + " (" + ast.body.length + ")");
    ...
    for (key in ast) {
      if (origin_obj_hasOwnProperty(ast, key)) {
        child = ast[key];
