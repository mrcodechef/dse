...
parser.addArgument(['--inlineIID'], {help: "Inline IID to (beginLineNo, beginColNo, endLineNo, endColNo) in JALANGI_$.iids in the instrumented file", action: 'storeTrue'});
parser.addArgument(['--outfile'], {help: "Output filename", action: 'append'});
parser.addArgument(['--inlineSource'], {help: "Inline original source as string in JALANGI_$.iids.code in the instrumented file", action: 'storeTrue'});
...
if (args.script_and_args.length === 0) {
    safe_print("must provide script to record");
    process.exit(1);
}
...
try {
  ...
  babel.transform('', { presets: ['@babel/preset-env'],
		...
		}); /* Precaches all dependencies o they dont get jalangi eval'd */
} catch (e) { safe_print("Error: Cannot load Babel!");}
...
Module._extensions['.js'] = function (module, filename) {
	...
	safe_print("FILE_OPEN: " + filename);

    var code = fs.readFileSync(filename, 'utf8');
	if (filename.includes('node_modules/@babel/helpers/lib/') || filename.includes('node_modules/lodash/') || filename.includes('node_modules/'))
		module._compile(code, filename);
	else
   {	safe_print("Instrumentation Start...")
		var instFilename = (args.outfile !== null ? args.outfile[0] : makeInstCodeFileName(filename));
		safe_print("Inst Filename: " + instFilename);
		var instCodeAndData = JALANGI_$.instrumentCode(
        {
            code: code,
            isEval: false,
	...
	safe_print("Instrumentation Finished")
    instUtil.applyASTHandler(instCodeAndData, astHandler, JALANGI_$);
...
function startProgram() {
    ...
    process.on('exit', function () { JALANGI_$.endExecution(); });
    Module.Module.runMain(script, null, true);
}

if (JALANGI_$.analysis && JALANGI_$.analysis.onReady) {
    JALANGI_$.analysis.onReady(startProgram);
} else {
    startProgram();
}
