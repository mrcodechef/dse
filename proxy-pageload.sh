...
if [ "$proto" != "" ]; then 
   node har --headless --url $1 --start-inject-js $2 --end-inject-js end-inject.js --proxy-address $5 > $outfile 2> /dev/null
	#./har-analyze.sh $1 8182 >  $3
	#node har $1 $2 8182 > out #| tee -a out
else
   node har --headless --url https://$1 --start-inject-js $2 --end-inject-js end-inject.js --proxy-address $5 > $outfile 2> /dev/null
